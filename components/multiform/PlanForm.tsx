...
import { FormItems } from "@/components/entry/multiform";
import arcadeImg from "@/public/imgs/arcade.png";
import advancedImg from "@/public/imgs/game-console.png";
import proImg from "@/public/imgs/online-gaming.png";

type stepProps = FormItems & {
  updateForm: (fieldToUpdate: Partial<FormItems>) => void;
};

type Plan = "arcade" | "advanced";

const PlanForm = ({ updateForm, plan, yearly }: stepProps) => {
  const [yearlyUpdated, setYearlyUpdated] = useState(yearly);
  const [planSelected, setPlanSelected] = useState<Plan | "pro">(plan);
  ...
  const handleValueChange = (planSelected: Plan | "pro") => {
    if (planSelected) {
  ...
  return (
    ...
    >
      <ToggleGroup.Root
        ...
        className="flex flex-col gap-3 my-2 md:flex-row md:items-center justify-center md:gap-8"
        type="single"
        value={planSelected}
      ...
      >
        ...
        >
          ...
          <div className="relative -top-1 flex flex-col items-start md:top-0">
            ...
            <p className="text-sm text-white">{yearly ? "$90/yr" : "$9/mo"}</p>
            {yearly && (
